...
#ifndef __FAST_PD_CF_H__
#define __FAST_PD_CF_H__
...
class CV_Fast_PD_cf
{
    public:
        ...
        ///*CV_Fast_PD( int numpoints, int numlabels, Real *lcosts,
              ...
              //      Real *wcosts , Real *primal0 = NULL )*/

          
        CV_Fast_PD_cf(int numpoints, int numlabels, Real *lcosts, int numpairs, int *pairs, 
            CostFunction *costFct, int max_iters, int  *primal0=NULL)
            ...
            // costFct = pointer to CostFunction class must have the method : computeDistance(pairindex i,label(p1),label(p2))
...

        {
            ...
            // printf("%d %d %f %d %d %f %d %d\n",
                // numpoints, numlabels, lcosts[numpoints*numlabels-2],
                // numpairs, pairs[numpairs*2-2], 
                // costFct->computeDistance(0,0,1),max_iters,
                // primal0[numpoints-2]);
            ...
            {
                _all_graphs[i] = new Graph( 
                    &_all_nodes[_numpoints*i], 
                    &_all_arcs[_numpairs*i<<1], 
                    _numpoints, 
                    err_fun );
    ...
    private:
        ...
        //
        CV_Fast_PD_cf( const CV_Fast_PD_cf &other );
        CV_Fast_PD_cf operator=( const CV_Fast_PD_cf &other );
};
